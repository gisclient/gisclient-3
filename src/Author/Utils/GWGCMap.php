...
class GWGCMap
{
    ...
    public function __construct($mapsetName, $getLegend = false, $languageId = null)
    {
        ...
        $this->getResolutions(
            $row["minscale"],
            empty($row["maxscale"]) ? $row["max_extent_scale"] : $row["maxscale"],
            $row["mapset_scales"]
        );
        ...
        if (!isset($this->tilesExtent)) {
            $this->tilesExtent = $this->getExtent($row["xc"], $row["yc"], $this->scaleListResolutions[0]);
        }
        $mapOptions["tilesExtent"] = $this->tilesExtent;
    ...
    private function getLayers()
    {
        ...
        $sqlLayers = "
            SELECT
                theme_id,theme_name,theme_title,theme_single,theme.radio,
                theme.copyright_string,layergroup.*,mapset_layergroup.*,
                outputformat_mimetype,outputformat_extension,
                owstype_name
            ...
            WHERE layergroup_id IN (
                SELECT layergroup_id FROM ".DB_SCHEMA.".layer
...
/*          if($row['refmap']){
    ...
    private function getFeatureTypes()
    {
        ...
        $sql = "SELECT
                    ...
                    data_unique, data_geom, layer.data, catalog.catalog_id, catalog.catalog_url,
                    private, layertype_id, classitem, labelitem, maxvectfeatures, zoom_buffer,
                    selection_color, selection_width, field_id, field_name, filter_field_name, field_header,
                    fieldtype_id, relation_name, relation_title, relationtype_id, searchtype_id, resultype_id,
                    datatype_id, field_filter, layer.hidden, field.editable as field_editable,
                    field_groups.groupname as field_group,field_groups.editable as group_editable,
