...
class PrintDocument
{
    ...
    public function __construct()
    {
        ...
        if (substr($this->options['TMP_PATH'], -1) != '/') {
        ...
        if (!isset($this->dimensions[$this->options['direction']])) {
            throw new \Exception('Invalid direction');
        }
        if (!isset($this->dimensions[$this->options['direction']][$this->options['format']])) {
    ...
    protected function getLegendsFromMapfile()
    {
        ...
        foreach ($this->tiles as $wms) {
            ...
            if (!empty($wms['parameters']['MAP']) && empty($mapset)) {
                $mapset = $wms['parameters']['MAP'];
            }
            ...
            foreach ($theme['groups'] as $group) {
                ...
                foreach ($group['layers'] as $key => $layer) {
                    ...
                    if (!isset($legendImages[$layer['url']])) {
                        ...
                        $legendImages[$layer['url']] = $this->getLegendImageWMS(
                            $group['id'],
                            $group['id'],
                            $tmpFileId,
                            $sld
            ...
            foreach ($this->legendArray as $theme) {
                ...
                foreach ($theme['groups'] as $group) {
                    foreach ($group['layers'] as $layer) {
                        if ($i % 3 == 0) {
                            $dom_grp_block = $dom_group->appendChild(new \DOMElement('group-block'));
                        }
                        $i++;
                        ...
                        if ($absoluteUrls) {
                            $layer['img'] = self::addPrefixToRelativeUrl($layer['img']);
                        }
