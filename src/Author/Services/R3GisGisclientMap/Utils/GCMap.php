...
namespace GisClient\Author\Services\R3GisGisclientMap\Utils;
...
class GCMap
{
    ...
    public $mapProviders = array(
        ...
        GMAP_LAYER_TYPE => "http://maps.google.com/maps/api/js?callback=GisClient.initMapset&sensor=false"
    );
    private $i18n;
    protected $oMap;
    ...
    public function __construct($mapsetName, $getLegend = false, $languageId = null, $onlyPublicLayers = false)
    {
        ...
        if ($row["mapset_srid"] == 4326) {
            $sizeUnitId = 7; //Forzo dd se in 4326
        }
        ...
        if (!empty($row["project_title"])) {
            $mapOptions["projectTitle"] = (strtoupper(CHAR_SET) != 'UTF-8') ? utf8_encode($row["project_title"]) : $row["project_title"];
        }
            ...
            if ($layerType == WMS_LAYER_TYPE) {
                ...
                if ($row["theme_single"] == 1) {
                    ...
                    if (empty($aLayers[$themeName]["type"])) {
                        ...
                        if ($row["layergroup_maxscale"] > 0 || $row["layergroup_maxscale"] < $aLayers[$themeName]["options"]["minScale"]) {
                            $layerOptions["minScale"] = intval($row["layergroup_maxscale"]);
                        }
                        if ($row["layergroup_minscale"] > 0 || $row["layergroup_minscale"] > $aLayers[$themeName]["options"]["maxScale"]) {
                        ...
                        if (isset($featureTypes['theme_' . $row['theme_id']])) {
                            $aLayers[$themeName]["options"]['featureTypes'] = array_values($featureTypes['theme_' . $row['theme_id']]);
                        }
                    }
                } else {
                    ...
                    if (isset($featureTypes['layergroup_' . $row['layergroup_id']])) {
                        $layerOptions['featureTypes'] = array_values($featureTypes['layergroup_' . $row['layergroup_id']]);
                    }
    ...
    public function _getLegendArray($layergroupId)
    {
    ...
    public function _getFeatureTypes()
    {
...
//print_r($_SESSION['GISCLIENT_USER_LAYER']);
    ...
    public function _getSelgroup()
    {
        ...
        for ($i = 0; $i < count($rowset); $i++) {
            if (!empty($this->i18n)) {
                $rowset[$i]['selgroup_title'] = $this->i18n->translate($rowset[$i]['selgroup_title'], 'selgroup', $rowset[$i]['selgroup_id'], 'selgroup_title');
            }
    ...
    public function _getTMSExtent($tilesExtent, $tilesExtentSRID)
    {
    ...
    public function OLMap()
    {
        ...
        if (!empty($this->mapOptions['displayProjection'])) {
            $mapOptions[] = '"displayProjection":new OpenLayers.Projection("' . $this->mapOptions['displayProjection'] . '")';
        }
        ...
        if (!empty($this->mapOptions['restrictedExtent'])) {
            $mapOptions[] = '"restrictedExtent":new OpenLayers.Bounds(' . implode(',', $this->mapOptions['restrictedExtent']) . ')';
        }
    ...
    public function _getProjectionDescription($authName, $authSrid)
    {
    ...
    public function _getExtent($xCenter, $yCenter, $Resolution)
    {
        //4tiles
        $aExtent = array();
    ...
    public function _getUsercontext($contextId)
    {
        $user = new \GCUser();
    ...
    public function _getMaxExtents()
    {
        $user = new \GCUser();
        $extents = array();
        $userGroupFilter = '';
