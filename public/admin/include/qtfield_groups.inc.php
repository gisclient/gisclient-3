...
$qtfield=$this->parametri["qtfield"];
...
$sql="SELECT X.groupname,CASE WHEN (SELECT count(*)=0 FROM ".DB_SCHEMA.".qtfield_groups  WHERE qtfield_id=$qtfield) THEN 1 WHEN coalesce(Y.groupname,'')='' THEN 0 ELSE 1 end as presente,coalesce(editable,0) as editable FROM ".DB_SCHEMA.".groups X LEFT JOIN (SELECT groupname,editable FROM ".DB_SCHEMA.".qtfield_groups  WHERE qtfield_id=$qtfield) Y USING(groupname);";
$msg=null;
if($db->sql_query($sql)){
