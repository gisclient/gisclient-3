...
if (empty($_REQUEST['layergroup_id'])) {
    die("Missing required parameter 'layergroup_id'");
}
...
if (empty($mapConfig['project_srid'])) {
    die('Missing project srid');
}
if (empty($mapConfig['xc']) || empty($mapConfig['yc'])) {
    die('Missing project center');
}
if (empty($mapConfig['max_extent_scale'])) {
    die('Missing project max extent');
}
