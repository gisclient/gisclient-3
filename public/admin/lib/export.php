...
function import($f, $parentId, $parentName, $newName = '', $parentkey = null)
{
    ...
    for ($i = 3; $i < count($rows); $i++) {
        ...
        if (preg_match_all('|@KEY\[(.+)\]\[(.+)\]@|Ui', $sql, $newkey)) {
            for ($j = 0; $j < count($newkey[0]); $j++) {
                if (!$newid[$newkey[1][$j]][$newkey[2][$j]]) {
                    $newid[$newkey[1][$j]][$newkey[2][$j]] = $newkey[2][$j];
                }
        ...
        if (preg_match("|@NEWKEY_I\[(.+)\]\[(.+)\]@|Ui", $sql, $out)) {
            ...
            if (in_array($out[1], ["username", "group"])) {
    ...
    if ($level == 'project') {
        ...
        while (trim($pk["parent"][$level])) {
            ...
            if (count($pk['pkey'][$level]) > 1) {
                ...
                if ($pk["parent"][$level]) {
                    $parentPK = $pk["pkey"][$pk["parent"][$level]][0];
                } else {
                    $parentPK = "null";
                }
...
function _export($fileName = "export.sql", $currentLevel, $projName, $structure, $start = 0, $startName, $parentValue, &$valutatedKey = null, &$Errors = [])
{
    ...
    for ($i = 0; $i < count($recordSet); $i++) {    //RISULTATI DA INSERIRE NEL FILE
        ...
        if (!fwrite($f, $sql)) {
            echo "<p>IMPOSSIBILE SCRIVERE SUL FILE $fileName:<br>$sql</p>";
        }
        fclose($f);
        //CHIAMATA RICORSIVA
        if ($child) {
...
function _exportNew($fileName = "export.sql", $arr, $lev, $project, $start = 0, $startName = '', $parentValue = [], &$valutatedKey)
{
    ...
    if ($parent_key && !$start) {                                          //FILTRO PER RECUPERARE I DATI DEL LIVELLO CORRENTE
        for ($i = 0; $i < count($parent_key); $i++) {
            $filter[] = "$parent_key[$i]='" . $parentValue[$parent_key[$i]] . "'";
        }
    } else {
        for ($i = 0; $i < count($pkey[$el["name"]]); $i++) {
        ...
        foreach ($rec as $key => $val) {        //Ciclo su tutti i campi
            ...
            if (in_array($key, $parent_key)) {                                 //CHIAVI DEL PARENT
                if (preg_match('/(.+)_id$/Ui', $key, $out)) {
                    $table = $out[1];
                    $name = _getFieldValue($table, $table . "_name", [$key], [$val]);
