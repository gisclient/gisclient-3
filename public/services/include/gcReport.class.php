...
class gcReport {
    ...
    function _initQuery($request) {
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			$qField[$Id][$fieldId]["field_name"]=isset($row["qt_field_name"])?trim($row["qt_field_name"]):'';
			$qField[$Id][$fieldId]["field_alias"]=isset($row["field_header"])?trim($row["field_header"]):'';
            $qField[$Id][$fieldId]["title"]=isset($row["field_header"])?trim($row["field_header"]):'';
			$qField[$Id][$fieldId]["formula"]=isset($row["formula"])?trim($row["formula"]):'';
			...
			$qField[$Id][$fieldId]["search_type"]=isset($row["searchtype_id"])?trim($row["searchtype_id"]):'';
			$qField[$Id][$fieldId]["result_type"]=isset($row["resultype_id"])?trim($row["resultype_id"]):'';
			$qField[$Id][$fieldId]["field_filter"]=isset($row["field_filter"])?trim($row["field_filter"]):'';
			...
			$f=array();
				...
				$qRelation[$Id][$relationId]["name"]=isset($row["qt_relation_name"])?trim($row["qt_relation_name"]):'';
				$qRelation[$Id][$relationId]["table_name"]=isset($row["table_name"])?trim($row["table_name"]):'';
				$qRelation[$Id][$relationId]["path"]=isset($row["catalog_path"])?trim($row["catalog_path"]):'';
				$qRelation[$Id][$relationId]["catalog_url"]=isset($row["catalog_url"])?trim($row["catalog_url"]):'';
