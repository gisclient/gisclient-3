...
if (!empty($_REQUEST['fixed_size'])) {
    $options['fixed_size'] = $_REQUEST['fixed_size'];
}
if (!empty($_REQUEST['scale'])) {
    $options['scale'] = $_REQUEST['scale'];
} else {
    $scaleWidth = (int) $options['dpi'] * 100 / 2.54 * ($options['extent'][2] - $options['extent'][0]) / $viewport_size[0];
    $scaleHeight = (int) $options['dpi'] * 100 / 2.54 * ($options['extent'][3] - $options['extent'][1]) / $viewport_size[1];
