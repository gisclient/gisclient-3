...
if (!isset($_REQUEST['REQUEST']) || !in_array($_REQUEST['REQUEST'], ['GetMap', 'SaveLayer', 'DeleteLayer', 'GetLayers', 'PrintMap'])) {
    die("REQUEST unknown");
}

if (!defined('REDLINE_TABLE') || !defined('REDLINE_SRID')) {
    outputError('Missing config redline values');
}
if (!defined('REDLINE_SCHEMA')) {
    define('REDLINE_SCHEMA', 'public');
}
if (!defined('REDLINE_FONT')) {
    define('REDLINE_FONT', 'arial');
}
if (!defined('POSTGIS_TRANSFORM_GEOMETRY')) {
...
if ($mapSRID != REDLINE_SRID) {
    ...
    if (!isset($projectInfo[$_REQUEST['PROJECT']]["PROJPARAMS"][$mapSRID])) {
        getProjParams($mapSRID);
    }
    if (!isset($projectInfo[$_REQUEST['PROJECT']]["PROJPARAMS"][REDLINE_SRID])) {
        getProjParams(REDLINE_SRID);
    }
