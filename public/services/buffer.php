...
if (empty($_REQUEST['features'])) {
    die(json_encode(array('result'=>'error', 'error'=>'missing features')));
}
if (empty($_REQUEST['projection'])) {
    die(json_encode(array('result'=>'error', 'error'=>'missing projection')));
}
...
if (!is_numeric($_REQUEST['buffer'])) {
    die(json_encode(array('result'=>'error', 'error'=>'invalid buffer')));
}
...
if (empty($bufferedGeoms)) {
    die(json_encode(array('result'=>'error', 'error'=>'empty buffered geoms', 'sql'=>$sql, 'params'=>$params)));
}
