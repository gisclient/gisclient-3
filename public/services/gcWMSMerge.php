...
} elseif (!empty($mapConfig['format']) && $mapConfig['format'] == 'jpeg') {
...
$sessionId = null;
if (isset($mapConfig['GC_SESSION_ID']) && !empty($mapConfig['GC_SESSION_ID'])) {
    $sessionId = $mapConfig['GC_SESSION_ID'];
}
        ...
        switch ($layer['SERVICE']) {
            ...
            case 'TMS':
                ...
                if (!empty($layer['PARAMETERS']['LAYERS'])) {
                    if (is_array($layer['PARAMETERS']['LAYERS'])) {
                        $layerNames = implode(',', $layer['PARAMETERS']['LAYERS']);
                    } else {
                        $layerNames = $layer['PARAMETERS']['LAYERS'];
                    }
                }
