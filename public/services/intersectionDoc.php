...
if (empty($catalogPath)) {
    $ajax->error('Undefined catalog '.$_REQUEST['catalog']);
}

$dataDb = GCApp::getDataDB($catalogPath);
$dbParams = GCApp::getDataDBParams($catalogPath);

if (empty($_REQUEST['docOptions']) || !is_array($_REQUEST['docOptions'])) {
    die('empty or invalid doc options');
}
$docOptions = $_REQUEST['docOptions'];
if (empty($docOptions['template']) || !file_exists(ROOT_PATH.'config/'.$docOptions['template'])) {
    die('empty or non existing template');
}
...
if (!empty($docOptions['groupBy']) && is_array($docOptions['groupBy'])) {
    ...
    $catalogPath = $stmt->fetchColumn(0);
    if (empty($catalogPath)) {
        $ajax->error('Undefined catalog '.$groupBy['catalog']);
    }
    
    $groupByDb = GCApp::getDataDB($catalogPath);
    $groupByDbParams = GCApp::getDataDBParams($catalogPath);
    
    if (!GCApp::tableExists($groupByDb, $groupByDbParams['schema'], $groupBy['table'])) {
        die('table '.$groupBy['table'].' does not exist');
    }
    $columnNames = GCApp::getColumns($groupByDb, $groupByDbParams['schema'], $groupBy['table']);
    if (!in_array($groupBy['field'], $columnNames)) {
        $ajax->error('Column '.$groupBy['field'].' of table '.$groupBy['table'].' does not exist');
    }
...
if (!empty($_REQUEST['formValues']) && is_array($_REQUEST['formValues'])) {
    foreach ($_REQUEST['formValues'] as $field) {
        $assign['formValues'][$field['name']] = $field['value'];
    }
}
...
foreach ($_REQUEST['intersections'] as $intersection) {
    if (!GCApp::tableExists($dataDb, $dbParams['schema'], $intersection['tableName'])) {
        $ajax->error('Table '.$intersection['tableName'].' does not exist');
    }
    
    $columnNames = GCApp::getColumns($dataDb, $dbParams['schema'], $intersection['tableName']);
    if (!in_array($intersection['artField'], $columnNames)) {
        $ajax->error('Column '.$intersection['artField'].' of table '.$intersection['tableName'].' does not exist');
    }
    if (!in_array($intersection['descField'], $columnNames)) {
        $ajax->error('Column '.$intersection['descField'].' of table '.$intersection['tableName'].' does not exist');
    }
    ...
    foreach ($arts as $art) {
        $assign['arts'][str_replace('.', '_', $art['art_field'])] = null;
    }
}
...
foreach ($_REQUEST['intersections'] as $intersection) {
    ...
    if (!empty($intersection['areaOnly'])) {
        $sql .= ' and st_area(st_intersection(the_geom, :geom)) > '.$minIntersectionArea.' ';
    }
...
foreach ($assign['table']['rows'] as $key => $feature) {
    foreach ($feature['intersections'] as $intersection) {
        if (!isset($groups[$intersection['intersection_group']])) {
            $groups[$intersection['intersection_group']] = array();
        }
    }
}
...
foreach ($assign['table']['rows'] as &$row) {
    ...
    foreach ($row['intersections'] as $intersection) {
        ...
        if ($intersection['areaOnly']) {
            $arts[$intersection['art_field']]['intersection_ratio'] += $intersection['intersection_ratio'];
        } else {
            $arts[$intersection['art_field']]['intersection_ratio'] = 1;
        }
    }
