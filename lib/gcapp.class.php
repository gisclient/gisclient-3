...
class GCAuthor
{
    ...
    public static function buildFeatureQuery($aFeature, array $options = array())
    {
            ...
            foreach ($aFeature["fields"] as $idField => $aField) {
                ...
                if (!empty($aField['relation']) && ($aFeature["relation"][$aField["relation"]]["relation_type"] == 2)) {
                    if (!$options['include_1n_relations'] || $options['group_1n']) {
                        continue;
                    } elseif (!empty($options['show_relation'])) {
                        //se voglio vedere i dati della secondaria di una sola relazione, escludo i campi delle altre
                        if ($options['show_relation'] != $aFeature['relation'][$aField['relation']]['name']) {
                            continue;
                        }
                    }
                }
            ...
            if ($aRelation=$aFeature["relation"]) {
                foreach ($aRelation as $idrel => $rel) {
                    ...
                    if ($rel["relation_type"] == 2) {
                        if (!$options['include_1n_relations']) {
                            continue;
                        }
        ...
        if (!empty($fieldList)) {
            $datalayerTable .= ', '.implode(',', $fieldList);
        }
        $datalayerTable .= ' FROM '.$joinString;
        if (!empty($groupByFieldList)) {
            $datalayerTable .= ' group by '.DATALAYER_ALIAS_TABLE.'.'.$datalayerKey.', '.DATALAYER_ALIAS_TABLE.'.'.$datalayerGeom.', '. implode(', ', $groupByFieldList);
        }
    ...
    public static function getTabDir()
    {
        ...
        if (!is_dir(ROOT_PATH.$rel_dir)) {
            $rel_dir="config/tab/it/";
        }
        if (defined('TAB_DIR')) {
            $rel_dir="config/tab/".TAB_DIR."/";
        }
        return $rel_dir;
    }
