...
CREATE OR REPLACE VIEW gisclient_34.seldb_relation AS
...
CREATE OR REPLACE VIEW gisclient_34.vista_field AS
       ...
       JOIN (         SELECT 0 AS relationtype_id, ''::character varying AS relationtype_name
               UNION
                        SELECT e_relationtype.relationtype_id, e_relationtype.relationtype_name
                          FROM gisclient_34.e_relationtype) z USING (relationtype_id)) x ON COALESCE(field.relation_id, 0) = x.relation_id
   JOIN gisclient_34.layer l USING (layer_id)
   JOIN gisclient_34.catalog c USING (catalog_id)
   LEFT JOIN gisclient_34.relation r ON r.relation_id = field.relation_id
