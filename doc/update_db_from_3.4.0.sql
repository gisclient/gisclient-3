...
------------------------------------------- INIZIO SVILUPPI AUTHOR 3.4 -------------------------------------------
    ...
    IF v_author_version = '3.6.3' THEN
        ...
        BEGIN
            ...
            EXECUTE format(
                'ALTER TABLE %I.project_srs ADD COLUMN id integer GENERATED BY DEFAULT AS IDENTITY',
                v_schema
            );
            ...
            EXECUTE format(
                'ALTER TABLE %I.project_srs ADD CONSTRAINT project_srs_project_name_srid_key UNIQUE (project_name, srid)',
                v_schema
            );

            EXECUTE format(
                'ALTER TABLE %I.mapset_layergroup ADD COLUMN id integer GENERATED BY DEFAULT AS IDENTITY',
                v_schema
            );
            ...
            EXECUTE format(
                'ALTER TABLE %I.mapset_layergroup ADD CONSTRAINT mapset_layergroup_mapset_name_layergroup_id_key UNIQUE (mapset_name, layergroup_id)',
                v_schema
            );
        END;
